...
public class GUI_RechtwinkligesDreieck_Mit_Berechnungsmethoden extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					GUI_RechtwinkligesDreieck_Mit_Berechnungsmethoden frame = new GUI_RechtwinkligesDreieck_Mit_Berechnungsmethoden();
	 ...
	 * Create the frame.
	 */
	public GUI_RechtwinkligesDreieck_Mit_Berechnungsmethoden() {
	...
	private void berechnen()
	{
		...
		// Eingabe
		a =     Double.parseDouble(tfKatheteA.getText()      )  ;    // Double.parseDouble() wandelt einen String um in Double-Datentyp
		c =     Double.parseDouble(tfHypotenuse.getText()       )  ;

		if (c > a)  // ok
		{
			//b = berechneKathete_b(a, c);
			b = TEST.berechneKathete_b(a, c);
			//U = berechneUmfang(a, b, c);
			U = TEST.berechneUmfang(a, b, c);
			//A = berechneFlaeche(a, b);
			A = TEST.berechneFlaeche(a, b);
			//alpha =  Math.toDegrees(    Math.atan(a / b)    )   ;
			alpha = TEST.berechneAPlha(a, b);
			//beta =  Math.toDegrees(    Math.acos(a / c)        );
			beta = TEST.berechneBeta(a, c);
		...
		else
		{
			...
			tfHypotenuse.setText(nk3.format(a));
	...
	/*
	private double berechneUmfang(double pa, double pb, double pc) {
		double ergebnis;
		ergebnis = pa + pb + pc;
		return ergebnis;
	}
	private double berechneFlaeche(double pa, double pb) {
		double Flacche;
		Flacche = pa * pb * 0.5;
		return Flacche;
	...
	private double berechneKathete_b(double pa, double pc) {
		double ergebnis;
		ergebnis = Math.sqrt(Math.pow(pc,2) - Math.pow(pa, 2));
		return ergebnis;
	}

	private double berechneAPlha(double pa, double pb) {
		double ergebnis;
		ergebnis = Math.toDegrees(    Math.atan(pa / pb)    );
		return ergebnis;
	}

	private double berechneBeta(double pa, double pc) {
		double ergebnis;
