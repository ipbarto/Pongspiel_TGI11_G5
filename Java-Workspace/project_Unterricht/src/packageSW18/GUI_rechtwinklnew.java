...
public class GUI_rechtwinklnew extends JFrame {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					GUI_rechtwinklnew frame = new GUI_rechtwinklnew();
		...
		Parallelschaltung_rechner rechner = new Parallelschaltung_rechner();
	}
	...
	public GUI_rechtwinklnew() {
		...
		JLabel lbWrong = new JLabel("Wrong numbers");
		
		lbWrong.setVisible(true);
		lbWrong.setForeground(Color.RED);
		lbWrong.setFont(new Font("Tahoma", Font.PLAIN, 18));
		lbWrong.setHorizontalAlignment(SwingConstants.CENTER);
		lbWrong.setBounds(264, 187, 143, 45);
		contentPane.add(lbWrong);
		
		JLabel lblNewLabel_1 = new JLabel("");
		lblNewLabel_1.setIcon(new ImageIcon(GUI_rechtwinklnew.class.getResource("/packageSW18/Parallelschaltung_Widerstände.svg.png")));
		lblNewLabel_1.setBounds(0, 213, 200, 160);
	...
	public void Berechne() {
		...
		U = Double.parseDouble(tfSpannung.getText());
		R1 = Double.parseDouble(tfWider1.getText());
		R2 = Double.parseDouble(tfWider2.getText());

		if (R1 != 0 && R2 != 0) {

				//Rges = R1 * R2 / ( (R1+R2));
				Rges = Parallelschaltung_rechner.GesWiderstand(R1, R2);
				//Iges = U / Rges;
				Iges = Parallelschaltung_rechner.GesStrom(U, Rges);
				//I1 = U / R1;
				I1 = Parallelschaltung_rechner.Strom1(U, R1);
				//I2 = U / R2;
				I2 = Parallelschaltung_rechner.Strom1(U, R2);
				//P = U * Iges;
				P = Parallelschaltung_rechner.Power(U, Iges);

				tfGesWider.setText(nk3.format(Rges));
				tfGesStrom.setText(nk3.format(Iges));
